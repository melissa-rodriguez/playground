...
class Ring {
  ...
  PVector surface(float theta, float phi) {
    ...
    float x = (r2+r1*easeInBack(cos(theta+t)))*cos(phi+t/2);
    float y = (r2+r1*easeInBack(cos(theta+t)))*sin(phi+t/2);
    float z = r2*easeInBack(sin(theta+t));
    float n = (float)noise.eval(x*0.002, y*0.002);

    return new PVector(x, y, z*n);
  }
}
