...
void drawCircle(float yHeight) {
  ...
  for (float a = 0; a< TWO_PI; a += 0.1) {
    ...
    float morph = map(sin(radians(frameCount*0.5)), -1, 1, 0, TAU);
    point(pow(sin(radians(y)), TAU*2)*r+5, cos(radians(y))*r); //change shape by varying TAU or PI
    point(-pow(sin(radians(y)), PI)*r-5, cos(radians(y))*r);  //making them equal = symmetry
