...
void drawSurface(float t) {
  ...
  for (int i = 0; i < total+1; i++) {
    ...
    for (int j = 0; j < total+1; j++) {
      ...
      globe[i][j] = new PVector(x, y+100*easeOutBack(sin(radians(mag(x,z))+t)), z);
    }
  }
...
void drawPoint(float t) {
  ...
  for (int i = 0; i < total+1; i++) {
    ...
    for (int j = 0; j < total+1; j++) {
      ...
      PVector v1 = new PVector(x, y+100*easeOutBack(sin(radians(mag(x,z))+t)), z);
      push();
      translate(v1.x, v1.y, v1.z);
      //circle(0, 0, 30);
      strokeWeight(20*(float)noise.eval(x,z));
      //strokeWeight(20*(float)noise.eval(x*0.02,z*0.02));
      //strokeWeight(40);
      point(0, 0, 0);
      pop();
    }
