...
void drawSurface(float t) {
  ...
  for (int i = 0; i < total+1; i++) {
    ...
    for (int j = 0; j < total+1; j++) {
      ...
      float x = r * sin(lat) * cos(lon);
      float y = r * sin(lat) * sin(lon);
      float z = r * cos(lat*easeInExpo(sin(radians(mag(x,y))+t)));
  ...
  for (int i = 0; i < total; i++) {
    ...
    for (int j = 0; j < total+1; j++) {
      ...
      stroke(255, 50*map(mag(v1.y, v1.y), 0, r, 0, 1));
      //stroke(255);
      strokeWeight(2);
        noStroke();
...
void drawPoint(float t) {
  ...
  for (int i = 0; i < total+1; i++) {
    ...
    for (int j = 0; j < total+1; j++) {
      float lon = map(j, 0, total, 0, TWO_PI);
      float x = (r+1) * sin(lat) * cos(lon);
      float y = (r+1) * sin(lat) * sin(lon);
      float z = (r+1) * cos(lat*easeInExpo(sin(radians(mag(x,y))+t)));
      //globe[i][j] = new PVector(x, y, z);
      PVector v1 = new PVector(x, y, z);
      push();
      translate(v1.x, v1.y, v1.z);
      //circle(0, 0, 30);
      //strokeWeight(10*(float)noise.eval(x,z));
      strokeWeight(7);
