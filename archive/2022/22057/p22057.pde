...
class Grid {
  ...
  void circleLine(int circleCount) {
    PVector startPoint = gridCells[int(random(cols+1))][0].cellPos;
    PVector nextPoint = startPoint; 
    ...
    for (int j = 0; j <= rows; j++) {
      ...
      for (int c = 0; c < circleCount; c++) {
        float cx = map(c, 0, circleCount, startPoint.x, nextPoint.x); 
        float cy = map(c, 0, circleCount, startPoint.y, nextPoint.y); 
        if (cy>grid_h || cy < 0) {
          break;
        }
        ...
        //if (rand == 0) {
        //  fill(map(dist(cx, cy, nextPoint.x, nextPoint.y), 0, res*2, 0, 1)*240);
        //} else {
        //  fill(map(dist(cx, cy, nextPoint.x, nextPoint.y), 0, res*2, 1, 0)*240);
        //}

        //fill(map(cy, 0, grid_h, 0, 1)*240); 
        //stroke(0); 
        //stroke(155, 155, sqrt(random(1))*255);
        fill(0); 
        circle(cx, cy, 5);

        //stroke(0); 
        if (c%2==1) {
          line(cx, cy, cx, cy+100*sqrt(random(1)));
        }
      ...
      if (startPoint.x>grid_w || startPoint.x < 0 || startPoint.y>grid_h || startPoint.y < 0) {
        break;
      }
    }
    endShape(OPEN);
