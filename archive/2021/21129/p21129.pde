...
PVector surface(float theta, float phi) {
  ...
  float x = scale*((3 + cos(theta+t / 2) * sin(phi) - sin(theta+t / 2) * sin(2 * phi)) * cos(theta));
  float y = scale*((3+ cos(theta+t / 2) * sin(phi) - sin(theta+t / 2) * sin(2 * phi)) * sin(theta));
  float z = scale*(sin(theta+t / 2) * sin(phi) + cos(theta+t / 2) * sin(2 * phi));

  //float x = scale*((3 + cos(theta / 2) * sin(phi) - sin(theta / 2) * sin(2 * phi)) * cos(theta));
  //float y = scale*((3+ cos(theta / 2) * sin(phi) - sin(theta / 2) * sin(2 * phi)) * sin(theta));
...
void drawPointsBack(float t) {
  int n1 = 10; // theta subdivisions
  int n2 = 100; // phi subdivisions
